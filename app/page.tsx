      ...
      <main id="top">
        ...
        <section className="directory" id="server-list">
          <div className="container">
            ...
            {directoryView !== "small" && <div className="list-head"><span><b>{filtered.length}</b>개 {directoryView === "new" ? "신규 서버" : "서버"}</span><div><span>반응형 홍보 배너</span><span>버전</span><span>접속자</span><span>추천</span></div></div>}

            {directoryView === "small" ? directoryLoading ? <div className="empty-state"><b>소규모 서버 통계를 계산하는 중</b><p>최근 7일 수집 기록을 확인하고 있습니다.</p></div> : filtered.length > 0 ? <div className="small-server-group"><div className="group-label"><b>SMALL SERVER RECOMMENDATION</b><span>온라인·신뢰도 우선 추천 · 평균 동접 20명 미만</span></div>{filtered.map((server, index) => <SmallServerRow key={server.id} server={server} rank={index + 1} favorite={favorites.includes(server.id)} onFavorite={() => toggleFavorite(server.id)} onCopy={() => copyText(server.address, `${server.address} 주소를 복사했습니다.`)} onOpen={() => void openServer(server)} />)}</div> : <div className="empty-state"><b>{smallServerPool.length === 0 ? "선정 기준을 충족한 서버가 없습니다" : "검색 결과 없음"}</b><p>{smallServerPool.length === 0 ? "상태 이력이 수집되고 7일 평균이 20명 미만이면 자동으로 추가됩니다." : "검색어 또는 필터를 변경해 보세요."}</p><button type="button" onClick={resetFilters}>필터 초기화</button></div> : <>
              {directoryView === "new" ? directoryLoading ? <div className="empty-state"><b>신규 서버를 확인하는 중</b><p>최근 7일 등록 기록을 불러오고 있습니다.</p></div> : filtered.length > 0 ? <div className="server-group new-server-group"><div className="group-label"><b>NEW SERVER ARRIVALS · 7 DAYS</b><span>최초 등록일 기준 최신순 · 공개 인증 서버만 표시</span></div>{filtered.map((server, index) => <ServerRow key={server.id} server={server} rank={index + 1} favorite={favorites.includes(server.id)} onFavorite={() => toggleFavorite(server.id)} onCopy={() => copyText(server.address, `${server.address} 주소를 복사했습니다.`)} onOpen={() => void openServer(server)} />)}</div> : <div className="empty-state"><b>{newServerPool.length === 0 ? "최근 7일 이내 등록된 서버가 없습니다" : "검색 결과 없음"}</b><p>{newServerPool.length === 0 ? "새 서버가 등록되고 공개 인증을 마치면 자동으로 이 목록에 추가됩니다." : "검색어 또는 필터를 변경해 보세요."}</p><button type="button" onClick={resetFilters}>필터 초기화</button></div> : <>
