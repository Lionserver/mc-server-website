  ...
  try {
    ...
    const [servers, blacklist, enforcements, conversations, audits, identities, announcements] = await Promise.all([
      ...
      environment.DB.prepare(`SELECT id, kind, value, reason, status, expires_at, created_by, created_at, updated_at
        FROM server_blacklist ORDER BY created_at DESC LIMIT 300`).all(),
      environment.DB.prepare(`SELECT e.id, e.server_id, e.kind, e.reason, e.status, e.starts_at, e.expires_at,
        ...
        ORDER BY CASE e.status WHEN 'active' THEN 0 ELSE 1 END, e.created_at DESC LIMIT 500`).all(),
    ...
    return Response.json({
      ...
      stats: {
        ...
        blacklistedServers: serverResults.filter((item) => item.status === "blacklisted").length,
        activeEnforcements: activeEnforcements.length,
        unreadMessages: conversations.results.reduce((sum, item) => sum + Number((item as { unread_admin?: number }).unread_admin ?? 0), 0),
        pendingOwnership: ownership.claims.filter((item) => item.status === "pending_review").length,
      },
