...
export const metadata: Metadata = {
  ...
  twitter: {
    card: "summary",
    title: "개인정보 처리방침 — Minecraft.kr",
    description: "Minecraft.kr 개인정보 처리방침",
  },
};
...
export default function PrivacyPage() {
  return <main className="policy-page"><article><Link className="policy-brand" href="/">Minecraft.kr</Link><span>PRIVACY POLICY</span><h1>개인정보 처리방침</h1>
    <p className="policy-lead">공고일·시행일: 2026년 7월 24일 · 베타 서비스 적용</p>
    ...
    <section><h2>1. 개인정보 처리 항목·목적·보유기간</h2><div className="policy-table-wrap"><table><thead><tr><th>구분</th><th>처리 항목</th><th>이용 목적</th><th>보유기간</th></tr></thead><tbody>
      ...
      <tr><td>사이트 접근 인증</td><td>ChatGPT Sites가 접근 허용 여부를 확인하는 과정에서 처리하는 OpenAI 계정 식별정보(이름, 이메일, 프로필 이미지 등). 서비스 애플리케이션에는 인증된 이메일만 전달되며 이름·프로필 이미지는 자체 데이터베이스에 저장하지 않음</td><td>비공개·제한 공개 사이트 접근권한 확인</td><td>ChatGPT Sites 이용계약과 OpenAI 정책에 따른 기간</td></tr>
      ...
      <tr><td>자동 생성 정보</td><td>서비스 접속 시각, 브라우저·기기 정보, 보안 로그, 쿠키, 일자별로 분리된 접속 네트워크 HMAC 대조값. 네트워크 제공자는 원문 IP를 일시 처리할 수 있음</td><td>보안, 장애 대응, 부정 이용 방지, 서비스 품질 유지, 한국시간 기준 일일 고유 방문 통계</td><td>일일 방문 대조값은 최대 3일, 추천 IP 대조정보는 90일. 그 밖의 정보는 목적 달성 또는 제공자 보관기간 종료 시까지</td></tr>
    ...
    <section><h2>3. 개인정보 처리업무 위탁</h2><p>서비스 제공에 필요한 업무를 아래 업체에 위탁합니다. 수탁자와 계약·약관을 통해 목적 외 처리금지, 안전조치, 재위탁 관리와 종료 시 삭제 의무를 확인합니다.</p><div className="policy-table-wrap"><table><thead><tr><th>수탁자</th><th>위탁 업무</th><th>처리 정보</th><th>보유 기준</th></tr></thead><tbody>
      ...
      <tr><td>OpenAI OpCo, LLC (ChatGPT Sites)</td><td>사이트 배포·운영, 유지보수, 지원과 접근 인증</td><td>호스팅된 서비스 데이터와 접속·운영 로그</td><td>서비스 계약 동안 처리하며 종료 시 이용자 지시와 법령에 따라 반환·삭제</td></tr>
    ...
    <section><h2>4. 개인정보의 국외 처리·이전</h2><p>호스팅과 이메일 발송은 서비스 계약의 체결·이행에 필요하며, 개인정보 보호법 제28조의8 제1항 제3호에 따라 아래 내용을 공개하고 국외 처리합니다.</p><div className="policy-table-wrap policy-wide-table"><table><thead><tr><th>이전받는 자·연락처</th><th>국가</th><th>항목·목적</th><th>시기·방법</th><th>보유 기준</th></tr></thead><tbody>
      ...
      <tr><td>OpenAI OpCo, LLC (ChatGPT Sites)<br/><a href="mailto:privacy@openai.com">privacy@openai.com</a></td><td>미국 및 서비스 제공을 위한 처리 소재국</td><td>호스팅된 서비스 데이터와 접속·운영 로그 · 사이트 배포·운영, 유지보수, 지원과 접근 인증</td><td>서비스 운영·접속 시 암호화된 네트워크로 지속 전송·처리</td><td>서비스 계약 동안 처리하며 종료 시 이용자 지시와 법령에 따라 반환·삭제</td></tr>
    ...
    </tbody></table></div><p className="policy-note">국외 처리를 원하지 않으면 개인정보 담당 창구로 계정 삭제와 처리정지를 요청할 수 있습니다. 다만 Cloudflare 또는 ChatGPT Sites 처리를 거부하면 웹서비스 이용이, Resend 처리를 거부하면 이메일 로그인과 소유권 알림 이용이 어렵습니다.</p></section>
    ...
    <section><h2>8. 안전성 확보조치</h2><ul><li>인증코드·세션·IP 대조값을 원문 대신 해시로 저장하고, 원문 IP는 서비스 데이터베이스에 저장하지 않습니다. 일일 방문 대조값은 날짜가 바뀔 때 서로 연결할 수 없도록 별도 키로 생성합니다.</li><li>HttpOnly·SameSite 쿠키, 동일 출처 검사, 요청 횟수 제한, 관리자 OTP, 일회용 실시간 접속권과 감사 로그를 적용합니다.</li><li>개인정보 접근 권한을 운영상 필요한 인원으로 제한하고, 전송구간 암호화와 저장소 접근 통제를 적용합니다.</li><li>HTML 직접 입력을 차단하고 업로드 파일 형식·크기 검증과 악성 입력 방어를 적용합니다.</li></ul></section>
