...
import { useSiteTraffic } from "@/components/site-traffic-provider";
...
export function HomeDirectory({
  ...
  const { todayVisitors, available: trafficAvailable } = useSiteTraffic();
  ...
  const [directoryLoading, setDirectoryLoading] = useState(!initialLoaded);
  const [directoryError, setDirectoryError] = useState("");
  const [directoryUpdatedAt, setDirectoryUpdatedAt] = useState<number | null>(initialGeneratedAt);
  ...
  useEffect(() => {
    ...
    const startPolling = () => {
      if (timer !== null || document.visibilityState !== "visible" || !navigator.onLine) return;
    ...
    const initial = initialLoaded ? null : window.setTimeout(() => void loadServers(), 0);
    startPolling();
    document.addEventListener("visibilitychange", syncPolling);
    window.addEventListener("online", syncPolling);
    window.addEventListener("offline", syncPolling);
    return () => {
      if (initial !== null) window.clearTimeout(initial);
      stopPolling();
      document.removeEventListener("visibilitychange", syncPolling);
      window.removeEventListener("online", syncPolling);
      window.removeEventListener("offline", syncPolling);
    };
  }, [initialLoaded, loadServers, refreshSelected]);
  ...
  useEffect(() => {
    ...
    let attempts = 0;
    let starting = false;
    const canConnect = () => active && navigator.onLine && document.visibilityState === "visible";
    ...
    const schedule = (callback: () => void, delay: number) => {
      clearReconnect();
      reconnect = window.setTimeout(callback, delay);
    };
    const connect = () => {
      if (socket) return;
      if (!canConnect()) {
      ...
      try {
        ...
        attempts += 1;
        setDirectoryConnection("polling");
        schedule(() => void start(), attempts >= 3 ? 5 * 60_000 : Math.min(10_000, 500 * 2 ** attempts));
      ...
      candidate.addEventListener("close", () => {
        ...
        attempts = openedAt > 0 && Date.now() - openedAt >= 30_000 ? 0 : attempts + 1;
        setDirectoryConnection("polling");
        if (attempts >= 3) {
          attempts = 0;
          schedule(() => void start(), 5 * 60_000);
        } else {
          schedule(connect, Math.min(10_000, 500 * 2 ** attempts));
        }
      });
    ...
    const pause = () => {
      ...
      socket = null;
      current?.close(1000, "page hidden or offline");
      setDirectoryConnection("polling");
    };
    const resume = () => {
        ...
        <section className="hero">
          <div className="container hero-layout">
            ...
            <div className="hero-search-wrap">
              ...
              <div className="index-status"><span><i /> 온라인 서버 <b>{formatPlayers(onlineCount)}</b></span><span>현재 접속자 <b>{formatPlayers(totalPlayers)}</b></span><span>{directoryConnection === "live" ? "실시간 LIVE" : "자동 갱신"} <b>{directoryUpdatedAt ? relativeTime(directoryUpdatedAt) : "불러오는 중"}</b></span><span className="today-visitor-stat" title="한국시간 기준 · 동일 네트워크는 하루 한 번 집계">오늘 방문자 <b>{todayVisitors === null ? trafficAvailable ? "집계 중" : "—" : `${formatPlayers(todayVisitors)}명`}</b></span></div>
            </div>
          </div>
        </section>
...
function ServerDetail({ server, returnFocusTo, onCopy, onMessage, onRefresh, onClaim }: {
  ...
  const ticketEmbed = `<iframe src="${PUBLIC_SITE_ORIGIN}/embed/server/${server.id}" title="${safeEmbedName} Minecraft.kr 서버 탑승권" width="760" height="190" loading="lazy" sandbox="allow-same-origin allow-popups allow-popups-to-escape-sandbox" referrerpolicy="strict-origin-when-cross-origin" style="width:100%;max-width:760px;height:190px;border:0;overflow:hidden;display:block"></iframe>`;
  const legacyEmbed = embedMediaType === "video/webm"
    ? `<a href="${PUBLIC_SITE_ORIGIN}/servers/${server.id}"><video src="${PUBLIC_SITE_ORIGIN}/api/servers/${server.id}/assets/desktopList" width="468" height="60" autoplay muted playsinline aria-label="${safeEmbedName} 서버"></video></a>`
    : `<a href="${PUBLIC_SITE_ORIGIN}/servers/${server.id}"><img src="${PUBLIC_SITE_ORIGIN}/api/servers/${server.id}/assets/desktopList" width="468" height="60" alt="${safeEmbedName} 서버"></a>`;
    ...
    <Dialog.Content className="detail-modal" aria-modal="true" aria-labelledby="detail-title" onCloseAutoFocus={(event) => {
      ...
      <div className="detail-shell">
        <div className="detail-summary">
          ...
          <div className="detail-quick-actions"><button type="button" className="address-hero" title={`${server.address} 복사`} onClick={() => onCopy(server.address, `${server.address} 주소를 복사했습니다.`)}><span><Signal size={14} /> {server.address}</span><b><Copy size={14} /> 복사</b></button><button type="button" className="vote-button" onClick={() => setVoteOpen(true)}><Trophy size={16} /> 오늘 추천하기</button>{server.discordEnabled && server.discordUrl && <a className="secondary-action" href={server.discordUrl} target="_blank" rel="ugc nofollow noopener noreferrer"><MessageCircle size={15} /> Discord</a>}{server.websiteEnabled && server.websiteUrl && <a className="secondary-action" href={server.websiteUrl} target="_blank" rel="ugc nofollow noopener noreferrer"><ExternalLink size={15} /> 웹사이트</a>}{server.kakaoEnabled && server.kakaoUrl && <a className="secondary-action" href={server.kakaoUrl} target="_blank" rel="ugc nofollow noopener noreferrer"><MessageCircle size={15} /> 카카오톡</a>}</div>
...
function renderDescriptionRuns(runs: DescriptionTextRun[], keyPrefix: string) {
  return runs.map((run, index) => run.href
    ? <a key={`${keyPrefix}-${index}`} href={run.href} target="_blank" rel="ugc nofollow noopener noreferrer" style={descriptionRunStyle(run)}>{run.text}</a>
...
function StaffDiscordContact({ value, nickname, onCopy }: { value: string; nickname: string; onCopy: (value: string, message: string) => void }) {
  ...
  if (href) return <a href={href} target="_blank" rel="ugc nofollow noopener noreferrer"><MessageCircle size={12} /> 개인 Discord 열기</a>;
...
function ServerRow({ server, rank, favorite, onFavorite, onCopy, onOpen }: { server: Server; rank?: number; favorite: boolean; onFavorite: () => void; onCopy: () => void; onOpen: () => void; }) {
  ...
  return <article className={server.sponsored ? "server-row sponsored-row" : "server-row"}>
    ...
    <div className="server-main"><div className="server-title"><a href={`/servers/${server.id}`} onClick={(event) => openServerModalFromLink(event, onOpen)}>{server.name}</a>{server.verified && <BadgeCheck size={14} aria-label="운영자 인증 완료" />}{server.tags.slice(0,2).map((tag) => <span key={tag}>{tag}</span>)}</div><p>{server.summary}</p><button className="address-button" type="button" onClick={onCopy}><Copy size={12} />{server.address}<span>복사</span></button></div>
    <a className="banner-open" href={`/servers/${server.id}`} onClick={(event) => openServerModalFromLink(event, onOpen)} aria-label={`${server.name} 배너로 상세보기`}><PromoBanner server={server} /></a>
    ...
    <div className="row-actions"><button type="button" className={favorite ? "favorite active" : "favorite"} aria-label={favorite ? `${server.name} 즐겨찾기 해제` : `${server.name} 즐겨찾기`} aria-pressed={favorite} onClick={onFavorite}><Star size={16} fill={favorite ? "currentColor" : "none"} /></button><a href={`/servers/${server.id}`} onClick={(event) => openServerModalFromLink(event, onOpen)} aria-label={`${server.name} 상세보기`}><ArrowUpRight size={17} /></a></div>
  </article>;
}
...
function SmallServerRow({ server, rank, favorite, onFavorite, onCopy, onOpen }: { server: Server; rank: number; favorite: boolean; onFavorite: () => void; onCopy: () => void; onOpen: () => void; }) {
  return <article className="small-server-row">
    ...
    <div className="small-row-main">
      ...
      <div className="server-title"><a href={`/servers/${server.id}`} onClick={(event) => openServerModalFromLink(event, onOpen)}>{server.name}</a>{server.tags.slice(0, 2).map((tag) => <span key={tag}>{tag}</span>)}</div>
      <p>{server.summary}</p>
      <button className="address-button" type="button" onClick={onCopy}><Copy size={12} />{server.address}<span>복사</span></button>
    </div>
    <a className="small-row-banner" href={`/servers/${server.id}`} onClick={(event) => openServerModalFromLink(event, onOpen)} aria-label={`${server.name} 배너로 상세보기`}><PromoBanner server={server} /></a>
...
function openServerModalFromLink(event: MouseEvent<HTMLAnchorElement>, onOpen: () => void) {
